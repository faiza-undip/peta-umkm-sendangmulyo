...
export default function AccountMenu({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-gold font-mono text-[10px] font-bold text-ink">
          {initials}
        </span>
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-64 rounded-sm border border-line bg-plate p-4 shadow-xl">
          ...
          <span className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-teal/15 px-2 py-1 font-mono text-[10px] uppercase tracking-wider text-teal">
            Admin
          </span>
          ...
          <form action={logout} className="mt-4 border-t border-line pt-4">
            <button
              type="submit"
              className="w-full rounded-sm border border-clay/50 px-3 py-2 text-left font-mono text-[11px] uppercase tracking-wider text-clay transition hover:bg-clay/10"
            >
              Keluar →
            </button>
          </form>
